...
void serverFunction(int connfd) {
    ...
    while (1) {                                                 // Infinite loop for menu implentation and user
        ...
        if (strncmp(buffer, "ADD_RECORD", 10) == 0) {
            ...
            char *result = searchByName(employees, count, firstName, lastName);             // Parse and pass the string into searchByName
            bzero(buffer, MAXLINE);
          ...
          char *result = searchByZipCode(employees, count, buffer);                         // Search for Zip Code entered in from client
            bzero(buffer, MAXLINE);
            ...
            char *result = searchBySalary(employees, count, salary, comparison);            // Search for salary
...
char* searchByZipCode(Struct_Employee_Info employees[], int count, const char *zipCode) {
    ...
    for (int i = 0; i < count; i++) {                           // Similar to the first function, append the found record to the result string if zip code found
